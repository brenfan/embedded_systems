...
static pthread_mutex_t soundBitesLock;
...
static int volume = 0;
void AudioMixer_setVolume(int a);
void AudioMixer_init(void)
{
	AudioMixer_setVolume(DEFAULT_VOLUME);
	pthread_mutex_init(&soundBitesLock, NULL);
	// Initialize the currently active sound-bites being played
	// REVISIT:- Implement this. Hint: set the pSound pointer to NULL for each
	//     sound bite.
	int i = MAX_SOUND_BITES;
	while (i--) {
	...
	// ..get info on the hardware buffers:
 	unsigned long unusedBufferSize = playbackBufferSize;
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	/* Thread safety on soundBites[] */
	pthread_mutex_lock(&soundBitesLock);
	/* 2. Search through the soundBites[] array looking for a free slot. */
	int spot = 0;
	while (soundBites[spot++].pSound != NULL && spot < MAX_SOUND_BITES);
	if (spot == MAX_SOUND_BITES){
		printf("Failed to queue sound (Max sounds queued)\n");
		pthread_mutex_unlock(&soundBitesLock);
		return;
	}

	/* 3. If a free slot is found, place the new sound file into that slot. */
	soundBites[spot].pSound = pSound;
	soundBites[spot].location = 0;

	pthread_mutex_unlock(&soundBitesLock);
	//printf("Queued a soundbite");
...
void AudioMixer_cleanup(void)
{
	...
	pthread_mutex_destroy(&soundBitesLock);
...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
	...
	while (i--)
		if(soundBites[i].pSound) {
			...
			int j = 0;
			while (j < size) {
				int temp = playbackBuffer[j] + (sound->pData[loc]);
